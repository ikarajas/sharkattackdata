(function($) {
	...
	Utils = {
		...
		getAttackCountsForYear: function(year, attacks) {
			yearSet = $.grep(attacks, function(a, i) {
				return a.date !== null && a.date.getFullYear() === year;
			});
			return {
				total: yearSet.length,
				fatal: $.grep(yearSet, function(a, i) {
					return a.fatal;
				}).length,
				unprovoked: $.grep(yearSet, function(a, i) {
					return a.unprovoked;
				}).length,
				unprovokedAndNonFatal: $.grep(yearSet, function(a, i) {
					return !a.fatal && a.unprovoked;
				}).length,
				fatalAndUnprovoked: $.grep(yearSet, function(a, i) {
					return a.fatal && a.unprovoked;
				}).length
			};
		},
	...
	$.widget("SharkAttackData.PlaceWidget", {
		_create: function() {
			...
			function AttackPlaceSummaryViewModel() {
				...
				self.isFiltered = ko.computed(function() {
					return self.filterDropdownFatalState() !== self.filterStatuses_fatal.fatal_and_non_fatal || self.selectedIncidentTypes().length < self.incidentTypes().length;
				});
		...
		_drawTimelineChart: function() {
			...
			var options = {
				...
				legend: {
					textStyle: widget.chartBaseTextStyle
				},
				vAxis: {
					textStyle: widget.chartBaseTextStyle,
					baselineColor: "#fff",
	...
	$.widget("SharkAttackData.WorldMapWidget", {
		_create: function() {
			...
			widget.vm.readyToChart.subscribe(function() {
				widget._drawMap();
			});
			...
			google.load("visualization", "1.0", {
				packages: ["geochart"],
				callback: function() {
					widget.vm.chartingApiLoaded(true);
				}
			});
